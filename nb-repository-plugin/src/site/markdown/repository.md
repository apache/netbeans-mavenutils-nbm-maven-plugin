---
title: HOWTO: Upload NetBeans release binaries to a Maven repository
author:
 - Milos Kleint
...
and remote repositories. See [`populate`](./populate-mojo.html) description.

For a binary-only upload (jar files and nbm files only), it's possible to download the bits from official sites.
See this page [Building from source](https://netbeans.apache.org/download/dev/index.html).

For the complete upload, you will need to checkout the sources of the IDE/Platform you
want to upload. See this page [Building from source](https://netbeans.apache.org/download/dev/index.html).
...
that shall be uploaded.
    ...
    Run `ant` in your local git clone to build the distribution at `nbbuild/netbeans` subdirectory.
