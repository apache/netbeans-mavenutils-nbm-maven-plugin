...
public class PopulateRepositoryMojo extends AbstractNetbeansMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (deployUrl != null) {
            deploymentRepository = repositorySystem.newDeploymentRepository(
                    session.getRepositorySession(),
                    new RemoteRepository.Builder(deployId, "default", deployUrl).build());
        ...
        while (tok.hasMoreTokens()) {
            ...
            String clust =
                    module.getAbsolutePath().substring(rootDir.getAbsolutePath().length() + 1);
            ...
            if (examinator.isNetBeansModule() || examinator.isOsgiBundle()) {
                ...
                String group = groupIdPrefix
                        + (examinator.isOsgiBundle()
                                ? GROUP_EXTERNAL
                                : examinator.hasPublicPackages() ? GROUP_API : GROUP_IMPL);
        ...
        try {
            for (Map.Entry<ModuleWrapper, Artifact> elem : tobePopulated.entrySet()) {
                ...
                if (!man.getModuleManifest().getClasspath().isEmpty()) {
                    try {
                        moduleJarMinusCP =
                                Files.createTempFile(man.getArtifact(), ".jar").toFile();
                    ...
                    try {
                        if (deploymentRepository != null) {
                            ...
                            deployRequest.addArtifact(
                                    art.setFile(moduleJarMinusCP != null ? moduleJarMinusCP : moduleJar));
                            if (javadoc != null) {
    ...
    void install(Artifact art) throws MojoExecutionException {
        try {
    ...
    private File createMavenProject(
            ModuleWrapper wrapper, List<ModuleWrapper> wrapperList, List<ExternalsWrapper> externalsList)
        ...
        if (!man.getDependencyTokens().isEmpty()) {
            for (String elem : man.getDependencyTokens()) {
                ...
                if (index > -1) {
                    ...
                    List<RemoteRepository> repos = repositorySystem.newResolutionRepositories(
                            session.getRepositorySession(),
                            Collections.singletonList(new RemoteRepository.Builder(
                                            dependencyRepositoryId, "default", dependencyRepositoryUrl)
                                    .build()));
                    ...
                    try {
                        artifactRequest.setArtifact(
                                new DefaultArtifact(groupIdPrefix + GROUP_API, artifactId, "", "pom", forcedVersion));
                        artifactResult =
                                repositorySystem.resolveArtifact(session.getRepositorySession(), artifactRequest);
                        dep.setGroupId(groupIdPrefix + GROUP_API);
                    } catch (ArtifactResolutionException x) {
                        try {
                            artifactRequest.setArtifact(new DefaultArtifact(
                                    groupIdPrefix + GROUP_IMPL, artifactId, "", "pom", forcedVersion));
                            artifactResult =
                                    repositorySystem.resolveArtifact(session.getRepositorySession(), artifactRequest);
                            ...
                            try {
                                artifactRequest.setArtifact(new DefaultArtifact(
                                        groupIdPrefix + GROUP_EXTERNAL, artifactId, "", "pom", forcedVersion));
        ...
        try {
            ...
            for (String string : content512) {
                ...
                if (split[0].equals(sha1) && split[1].contains(":")) {
                    ...
                    getLog().info("found match " + dep.getGroupId() + ":" + dep.getArtifactId() + ":" + dep.getVersion()
                            + " for " + f.getName());
                    return dep;
                }
            }
