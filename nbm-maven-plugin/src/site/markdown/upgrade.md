---
title: HOW TO: Migrate from older version of the plugin
author:
 - Milos Kleint
...
The jdk baseline is JDK 21.

## Upgrading to 14.4

In 14.4, the `build-installers` goal is removed as Apache NetBeans harness NBI related element are removed.
...
The values from descriptor are still applied, warnings are printed.
In future releases the parameter will be removed.

## Upgrading from 2.6 version to 3.0

There are a few significant incompatible changes introduced in **3.0's** version of NBM packaging lifecycle.
The result should be easier to setup builds and better support for building NetBeans platform based applications.
...
* The project's dependencies that shall end up on **Class-Path** of the module and now processed transitively.
In earlier versions, you had to either explicitly list all such items in the pom as direct dependencies, or list them in the module descriptor.
...
be included (unless defined in your project directly).
Possible trouble makers are transitive dependencies that are defined both in a depending module and a regular jar dependency.
Based on Maven dependency tree resolution, the binary could end up on the **Class-Path** or not. The resolution to the problem is to define
...
the [`dependency:analyze`](https://maven.apache.org/plugins/maven-dependency-plugin/analyze-mojo.html)
...
[issue](https://github.com/mojohaus/nbm-maven-plugin/issues/3) for details. If you have previously
...
Use the [`dependency:analyze`](https://maven.apache.org/plugins/maven-dependency-plugin/analyze-mojo.html) goal to check for used/unused direct dependencies.
...
will get higher than necessary. Again the [`dependency:analyze`](https://maven.apache.org/plugins/maven-dependency-plugin/analyze-mojo.html) goal shall help.
If you know the jar is required but is provided by a module you depend on, use the `provided` scope to prevent inclusion.
